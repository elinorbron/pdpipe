...
from pdpipe.cq import OfDtypes, WithMissingValues
...
class Scale(ColumnsBasedPipelineStage):
    ...
    **kwargs : extra keyword arguments
    ...
    def __init__(self, scaler, columns=None, exclude_columns=None, **kwargs):
        self.scaler = scaler
        self._kwargs = kwargs
        super_kwargs = {
            "columns": columns,
    ...
    def _fit_transform(self, df, verbose):
        ...
        unscaled_cols = [x for x in df.columns if x not in self._columns_to_scale]
        col_order = list(df.columns)
...
class TfidfVectorizeTokenLists(PdPipelineStage):
    ...
    def __init__(self, column, drop=True, hierarchical_labels=False, **kwargs):
        ...
        self._vectorizer_args = {
            k: kwargs[k]
            for k in kwargs
            if k in valid_vectorizer_args and k not in ["input", "analyzer", "self",]
        }
        pipeline_stage_args = {
    ...
    def _fit_transform(self, df, verbose):
        self._tfidf_vectorizer = TfidfVectorizer(
            input="content", analyzer=_identity_function, **self._vectorizer_args,
        )
